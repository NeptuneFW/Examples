...
class UserTable
{
    ...
    public function table($table) 
    {
        ...
        $columnSurname = \Triton::TableColumn("surname");
            ...
            $table->varchar($columnSurname,255);
            
        $columnCreated_time = \Triton::TableColumn("created_time");
            $columnCreated_time->null("NOT NULL"); 
            $columnCreated_time->default("current_timestamp()");
            $columnCreated_time->extra("on update current_timestamp()");
    ...
    public static function all($columns = null, $type = 'object') 
    {
        ...
        $selectWhere = $pdoConnection->query('SELECT ' . $columnString . ' FROM user ')->fetchAll();
        if($type == 'array') 
        {
            return $selectWhere;
        }
    ...
    public function execute() {
        $pdoConnection = $GLOBALS['Databases']['Ntblog'];
        $bind = [];
        $selectQuery = "SELECT * FROM user WHERE " . $this->where['first'][0] . " " . $this->where['first'][1] . ":first ";
        ...
        if(count($selectWhere) > 1) 
        {
            ...
            foreach ($selectWhere as $itKey => $itValue) 
            {
                foreach ($itValue as $itemKey => $itemValue) 
                {
                    ...
                    {
                        $classRow->triton_user_id = [$itemValue, self::$table_id_column];
                    }
                    if (!is_int($itemKey)) {
                        $classRow->$itemKey[] = $itemValue;
                    }
    ...
    {   
        ...
        if(empty($where))
        {
            $count = $pdoConnection->query("SELECT COUNT(id) FROM user")->fetchAll();
            return $count[0][0];
        }
        else 
        {
            $count = $pdoConnection->query("SELECT COUNT(id) FROM user WHERE " . $where .  " ")->fetchAll();
            return $count[0][0];
        }           
    }
    ...
    public static function add($id = null,$rank = null,$deleted = null,$name = null,$surname = null,$created_time = null)
    {
        $database = explode(DIRECTORY_SEPARATOR , __DIR__);
        $database = end($database);
        $table = explode(DIRECTORY_SEPARATOR, explode("Table" , __CLASS__)[0]);
