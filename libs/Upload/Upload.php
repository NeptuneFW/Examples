            ...
            if (in_array($file_ext, $allowed)){
                if ($file_error === 0){
                    if ($file_size <= $size){
                        if ($new_name == false){
                            $file_name = $name['name'];
                            $file_destination = ROOT . '/public/upload/' . $file_name;
                            if (move_uploaded_file($file_tmp, $file_destination)){
                                return '/public/upload/' . $file_name;
                            }
                        }else {
                            $file_name_new = uniqid('', true) . '.' . $file_ext;
                            $file_destination = ROOT . '/public/upload/' . $file_name_new;
                            if (move_uploaded_file($file_tmp, $file_destination)){
                                return '/public/upload/' . $file_name_new;
                            }
                        }
                    }
                }
